...
{
	//APP_DBG_MSG( "BSP_SPI1_SendRecv\r\n");
  int32_t ret = 0x0E;

 // APP_DBG_MSG( "BSP_SPI1_SendRecv len %d\r\n",len);
 // APP_DBG_MSG( "BSP_SPI1_SendRecv tx %02X\r\n",pTxData[0]);


  HAL_StatusTypeDef status =HAL_SPI_TransmitReceive(&hspi3, pTxData, pRxData, len, 1000);

//  APP_DBG_MSG( "BSP_SPI1_SendRecv %02x\r\n",status);
  if (status == HAL_OK)
  {
	  //APP_DBG_MSG( "BSP_SPI1_SendRecv\r\n");
    ret = len;
  }
 // APP_DBG_MSG( "BSP_SPI1_SendRecv rx %02X\r\n",pRxData[0]);
  return ret;
}
